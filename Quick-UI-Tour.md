...
You can open a Cuis image using a portable virtual machine and see the same windows in the same positions pixel-per-pixel on Linux, Mac, Windows or any operating system where the VM runs.

You can start an image, do things with it, and throw it away, or you can save the image and its record of the changes to be opened another time and pick up right where you left off.
...
The initial user interface screen is pretty sparse.  Cuis Smalltalk is an 'Interactive Development Environment' just waiting for you to explore.
...
You can select or change code in the Terse Guide pages and "DoIt" (cmd-d) or "PrintIt" (cmd-p) to see results.  The page is reset every time you revisit a topic, so feel free to play.
...
Moving left to right at the top, clicking on a Class Category shows related Classes. Clicking on a Class shows its code.  Selecting a Method Category shows related methods.  Selecting a Method name shows its code in the lower pane.
...
You can also write code in the lower pane where "self" is bound to the object clicked on in the upper left pane.  So one can do things like changing the setting of the "color" instance variable.
...
The upper-left pane of the Browser shows the Class inheritance hierarchy for a BoxedMorph.  Each "child" or specialization of a Class has code that essentially says, "I am like my parent but for these changes/extensions", which it then defines.
